...
window.onload = function(){
  document.getElementById("divOpen").onclick = function(){
    OpenDiv();
  }
...
function OpenDiv(){
  let value = document.querySelector('#divClose');
...
window.onload = function InformaAnoAtual() {           
  DataAtual = new Date();
  document.getElementById('AnoAtual').innerHTML = DataAtual.getFullYear();
}

$(document).ready(function(){
  $('#numberTelefone').mask('(00) 0 0000-0000 ');
});

$(document).ready(function(e) {
  $('#textEmail').change(function() {
      var sEmail = $('#textEmail').val();
      if ($.trim(sEmail).length == 0) {     
          $('#errorEmail').text('Please enter valid email address');
      }
      if (validateEmail(sEmail)) {        
          $('#errorEmail').text('e-mail ok');
          setTimeout(() => {
            $('#errorEmail').text('');
          }, 3500);
      }
